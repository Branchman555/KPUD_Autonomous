...
def crop(image, top_percent, bottom_percent):
    ...
    assert 0 <= bottom_percent < 0.5, 'bottom_percent should be between 0.0 and 0.5'
...
def resize(image, new_dim):
    ...
    :return:
        Resize image
    """
    return cv2.resize(image, dsize=new_dim, interpolation=cv2.INTER_CUBIC)
...
def get_next_image_files(batch_size=64):
    ...
    # head = bernoulli.rvs(0.6)
    # if head == 1:
    #     image, steering_angle = random_shear(image, steering_angle)

    zeros = data['x'][data['x'] == 0].sample(n=40)
    non_zeros = data['x'][data['x'] != 0].sample(n=24)

    image_files_and_angles = []
    for index in zeros.index:
        #img = cv2.imread(IMG_PATH + str(index) + '.jpg')
        img = str(index) + '.jpg'
        angle = data.iloc[index]['x']
        image_files_and_angles.append((img, angle))
    for index in non_zeros.index:
        #img = cv2.imread(IMG_PATH + str(index) + '.jpg')
        img = str(index) + '.jpg'
        angle = data.iloc[index]['x']
        image_files_and_angles.append((img, angle))
